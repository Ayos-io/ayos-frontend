...
async function getData() {
  ...
  results['blocklist'].forEach(e => {
    ...
    if (e['enabled'] === '1') {
      toggle.checked = true;
      document.getElementById(e['service']).classList = 'text-base enable-cursor text-gray-700 dark:text-zinc-200';
    } else {
      toggle.checked = false;
      document.getElementById(e['service']).classList = 'text-base enable-cursor text-gray-400 dark:text-zinc-400';
    }
  ...
  document.querySelector('#last-update').innerText = results['last_update'];
}

async function getStats() {
  const results = await fetchAPIData('/adblock/stats');
  document.querySelector('#blocks').innerHTML = results['block'];
}
